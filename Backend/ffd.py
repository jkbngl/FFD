...
def readAmounts(level_type, cost_type, parent_account, year, month, day, _type, groupBy):
	...
	#85 check to prohibit SQL injections, as the query is dynamically build and can not be done in the normal way - not relly needed as its already declaere in the swagger, just to be sure
        ...
        cursor.execute(f"select sum(amount) sum, year || '-' || month level1, -99 level1_fk, -1 level_type, year, month from ffd.{'act' if _type == 'actual' else 'bdg'}_data {where_params} group by year, month order by year, month desc")
