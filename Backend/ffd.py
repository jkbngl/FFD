...
def readAmounts(level_type, cost_type, parent_account, year, month, day, _type, groupBy):
	...
	#85 check to prohibit SQL injections, as the query is dynamically build and can not be done in the normal way - not relly needed as its already declaere in the swagger, just to be sure
        ...
        cursor.execute(f"select sum(amount) sum, year level1, -99 level1_fk, -1 level_type, year from ffd.{'act' if _type == 'actual' else 'bdg'}_data {where_params} group by year order by year desc")
        ...
        cursor.execute(f"select sum(amount) sum, year || '-' ||  LPAD(month::text, 2, '0') || '-' ||  LPAD(day::text, 2, '0') level1, -99 level1_fk, -1 level_type, year, month, day from ffd.{'act' if _type == 'actual' else 'bdg'}_data {where_params} group by year, month, day order by day desc")
